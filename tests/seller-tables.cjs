...
(async()=>{
 ...
 try{
  ...
  await page.locator('[data-document="example-1"]').click();await page.locator('#drawer').getByText('Образец документа из 1С',{exact:true}).waitFor();assert.equal(await page.locator('#exportDocument').textContent(),'Скачать образец накладной');
