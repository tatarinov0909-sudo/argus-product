(() => {
  ...
  $('connectForm').onsubmit=async e=>{e.preventDefault();const button=e.submitter;button.disabled=true;message('setupMessage','Проверяем бота…');
    ...
    try{const r=await api('/telegram/connect','POST',{token:secret});const start=new URL(r.url).searchParams.get('start');if(!start)throw Error('Не удалось подготовить ссылку для Telegram. Повторите подключение.');$('startBot').href=r.url;$('startCommand').textContent='/start '+start;$('confirmStep').hidden=false;message('setupMessage','Откройте бота и нажмите «Старт». Если приложение не передало ссылку, отправьте скопированную команду в чат с ботом.');}catch(e){message('setupMessage',e.message,true);}finally{button.disabled=false;}};
  $('copyStartCommand').onclick=()=>action($('copyStartCommand'),async()=>{const command=$('startCommand').textContent;if(!command)throw Error('Сначала начните подключение.');if(!navigator.clipboard?.writeText)throw Error('Скопируйте команду из строки вручную.');await navigator.clipboard.writeText(command);message('setupMessage','Команда скопирована. Вставьте её в чат с ботом и отправьте.');});
