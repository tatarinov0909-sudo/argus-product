...
(() => {
  ...
  async function api(path,options={}){
    ...
    const renewed=response.headers.get('X-Argus-Token');if(renewed){state.token=renewed;localStorage.setItem('argus_token',renewed);}
  ...
  async function navigate(refresh=false){
    ...
    try{
      ...
      if(state.view==='products')renderProducts();else if(state.view==='orders')renderOrders();else if(state.view==='supplies')renderSupplies();else renderDocuments();
  ...
  // Поставки на WB. Приход на склад — это «Приходы и документы»; здесь то,
  // что склад собрал из заказов покупателей и везёт на сортировочный центр.
  ...
  function openSupply(id){
    const r=(state.supplies||[]).find(x=>x.id===id);if(!r)return;openDrawer(r.number,'Поставка на WB');
    $('drawerBody').innerHTML=`<div class="drawer-meta"><span>${h(r.destination||'Пункт ещё не выбран')}</span>${badge(r.statusName)}</div>${r.mpBarcodeFile?`<div class="supply-qr-big"><img src="data:image/svg+xml;base64,${h(r.mpBarcodeFile)}" alt="QR поставки"><div><div class="small muted">Поставка на WB</div><b>${h(r.mpSupplyId||'')}</b></div></div>`:''}<div class="table-scroll"><table class="data-table"><thead><tr><th>Заказ</th><th>Товар</th><th class="num">Шт.</th><th>Статус</th></tr></thead><tbody>${r.items.map(i=>`<tr><td>${h(i.order)}</td><td>${h(productName({sku:i.sku,name:i.name}))}</td><td class="num">${n(i.qty)}</td><td>${h(i.status)}</td></tr>`).join('')}</tbody></table></div>`;
  }
